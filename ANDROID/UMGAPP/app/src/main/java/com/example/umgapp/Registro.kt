                ...
                if(txtcorreo.text.endsWith("@miumg.edu.gt")){
                    if(txtcontra.text.length>=6){
                        if(txtconfcontra.text.toString()==txtcontra.text.toString()){
                            ...
                            db.collection("codigoscat")
                                .whereEqualTo("codigo", txtcodcat.text.toString().toInt())
                                .get()
                                .addOnSuccessListener { documents ->
                                    for (document in documents) {
                                        cargo= document.get("cargo").toString().toInt()
                                        Toast.makeText(this, "cargo "+cargo, Toast.LENGTH_SHORT).show()
                                        encontrado++
                                        //Log.d(TAG, "${document.id} => ${document.data}")
                                    }
                                    if(encontrado==0){
                                        Toast.makeText(this, "El código de catedrático ingresado no existe", Toast.LENGTH_SHORT).show()
                                    }else{
                                        //Toast.makeText(this, "El código de catedrático ingresado si existe", Toast.LENGTH_SHORT).show()
